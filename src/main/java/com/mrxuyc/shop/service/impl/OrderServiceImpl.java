...
import com.mrxuyc.shop.dao.OrderItemMapper;
import com.mrxuyc.shop.dao.OrderMapper;
import com.mrxuyc.shop.dao.PayInfoMapper;
import com.mrxuyc.shop.dao.ProductMapper;
import com.mrxuyc.shop.dao.ShippingMapper;
import com.mrxuyc.shop.pojo.Cart;
import com.mrxuyc.shop.pojo.Order;
import com.mrxuyc.shop.pojo.OrderItem;
import com.mrxuyc.shop.pojo.PayInfo;
...
import com.mrxuyc.shop.vo.OrderItemVo;
import com.mrxuyc.shop.vo.OrderProductVo;
import com.mrxuyc.shop.vo.OrderVo;
import com.mrxuyc.shop.vo.ShippingVo;
...
public class OrderServiceImpl implements IOrderService{
    ...
    public ServerResponse pay(Long orderNo, Integer userId, String path) {
        ...
        String subject = new StringBuilder().append("shop扫码支付,订单号:").append(outTradeNo).toString();
    ...
    public ServerResponse alipayCallback(Map<String, String> params) {
        ...
        if(order == null){
            return ServerResponse.createByErrorMsg("非商城的订单,回调忽略");
        }
    ...
    public ServerResponse createOrder(Integer userId, Integer shippingId) {
        //获取购物车中勾选的商品
        List<Cart> carts=cartMapper.selectSelectCartByUserId(userId);
        //
        ...
        if (order==null){
            return ServerResponse.createByErrorMsg("订单生成错误");
        }
        for (OrderItem orderItem : orderItemList) {
        ...
        if (rowCount<=0){
            //插入失败
            return ServerResponse.createByErrorMsg("订单详情生成错误");
        }
        //减少库存
    ...
    public ServerResponse getOrderCartProduct(Integer userId) {
        ...
        //从购物车中获取数据
        List<Cart> cartList = cartMapper.selectSelectCartByUserId(userId);
    ...
    private OrderVo assembleOrderVo(Order order , List<OrderItem> orderItemList){
        ...
        orderVo.setPaymentTypeDes(Const.PaymentTypeEnum.codeOf(order.getPaymentType()).getValue());
        orderVo.setStatus(order.getStatus());
        orderVo.setStatusDes(Const.OrderStatusEnum.codeOf(order.getStatus()).getValue());
        orderVo.setPostage(order.getPostage());
        orderVo.setShippingId(order.getShippingId());
        Shipping shipping = shippingMapper.selectByShippingIdAndUserId(order.getUserId(), order.getShippingId());
        if (shipping!=null){
    ...
    private void reduceProductStock(List<OrderItem> orderItemList){
        for (OrderItem orderItem : orderItemList) {
            ...
            productMapper.updateProductCount(productId,quantity);
        }
    }
    ...
    private ServerResponse getCartOrderItem(Integer userId,List<Cart> carts){
        ...
        for (Cart cartItem : carts) {
            ...
            if (Const.ProductStatusEnum.ON_SALE.getCode()!=product.getStatus()){
                return ServerResponse.createByErrorMsg("产品不是在线售卖状态");
            }
